package cn.edu.mydotabuff.adapter;
    ...
    private FavoriteItem extractFavoriteItem(Cursor c) {
        ...
        try {
            if (item.type == FavoriteItem.KEY_TYPE_HERO) {
                item.heroData =
                        DataManager.getHeroItem(DotaApplication.getApplication(), item.keyName);
            } else if (item.type == FavoriteItem.KEY_TYPE_ITEMS) {
                item.itemsData =
                        DataManager.getItemsItem(DotaApplication.getApplication(), item.keyName);
            }
        } catch (IOException e) {
            e.printStackTrace();
