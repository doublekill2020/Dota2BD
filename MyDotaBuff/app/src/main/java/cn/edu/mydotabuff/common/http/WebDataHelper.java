                ...
                switch (type) {
                    ...
                    case DETAIL:
                        ...
                        try {
                            ...
                            if (doc != null) {
                                if (doc.select("div.container.xuning-box").size() > 0) {
                                    // 获取连胜连败
                                    Elements select = doc.select("div.container.xuning-box");

                                    Element child = select.first().children().get(0).child(0);
                                    Element childRecord = select.first().children().get(2);

                                    bean.setLoseStreak(child.child(0).select("span").text());
                                    bean.setWinStreak(child.child(2).select("span").text());

                                    // 获取最高记录
                                    ArrayList<BestRecord> beans = new ArrayList<BestRecord>();
                                    Element bestRecordDiv = childRecord.select(
